...
<Callout type="error">
从 `drizzle-orm@1.0.0-beta.15` 开始，`drizzle-zod` 已被弃用，推荐使用 Drizzle ORM 内置的一流模式生成功能

你仍然可以使用 `drizzle-zod` 包，但所有新更新将直接添加到 Drizzle ORM
</Callout>
...
const { createInsertSchema } = createSchemaFactory({
  // 这个配置只会强制转换日期。设置 `coerce` 为 `true` 以强制所有数据类型，或指定其他类型
  coerce: {
    date: true
  }
});
