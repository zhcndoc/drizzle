...
> 然后我将通过 Atlas 应用这些架构到我的数据库中。

<Callout collapsed="Expand details">
这是一种 **代码库优先** 的方法。你将 TypeScript Drizzle 架构作为事实来源， 
Drizzle 允许你基于模式更改生成 SQL 表示，使用 [`drizzle-kit export`](/docs/drizzle-kit-generate)，
然后你可以通过外部迁移工具将这些架构应用到数据库。
