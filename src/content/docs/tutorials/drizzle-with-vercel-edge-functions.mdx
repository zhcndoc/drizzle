...
- [Neon 无服务器驱动程序](/docs/get-started-postgresql#neon) 允许您通过 HTTP 或 WebSockets 从无服务器和 Edge 环境查询您的 Neon Postgres 数据库，而不是通过 TCP。我们建议使用此驱动程序连接到 `Neon Postgres`。
- [Vercel Postgres 驱动程序](/docs/get-started-postgresql#vercel-postgres) 构建于 `Neon 无服务器驱动程序` 之上。我们建议使用此驱动程序连接到 `Vercel Postgres`。
- [PlanetScale 无服务器驱动程序](/docs/mysql/get-started-mysql#planetscale) 允许您访问任何 `MySQL` 客户端，并通过 HTTP 连接执行查询，这通常不会被云服务提供商阻止。
- [libSQL 客户端](/docs/sqlite/get-started-sqlite#turso) 允许您访问 [Turso](https://docs.turso.tech/introduction) 数据库。
...
您可以查看文档中的 [快速入门指南](/docs/sqlite/get-started-sqlite#turso) 或 [教程](/docs/tutorials/drizzle-with-turso)，了解如何在 Turso 中使用 Drizzle。
