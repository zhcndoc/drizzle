---
title: "在 Railway 上使用 Bun 和 PostgreSQL 的 Drizzle"
date: "2026-03-07"
---
...
本教程演示如何在 [Railway](https://driz.link/railway) 上部署的 [Bun](https://bun.sh/) 运行时和 PostgreSQL 数据库中使用 Drizzle ORM。

<Prerequisites>
  - 你应该已经安装了 [Bun](https://bun.sh/)。你可以按照[官方指南](https://bun.sh/docs/installation)进行安装。
...
登录你的 [Railway 控制台](https://railway.app/dashboard)并点击 `New Project` 按钮。

![](@/assets/images/tutorials/railway-create-service-menu.png)

#### 配置 PostgreSQL 数据库

在项目画布中，点击右上角的 `New` 按钮，然后选择 `Database` → `PostgreSQL`。你也可以使用命令面板（`Cmd/Ctrl + K`）并搜索 PostgreSQL。Railway 将为你提供一个新的 PostgreSQL 实例。
...
对于**本地开发**（例如运行 `drizzle-kit push` 或 `drizzle-kit studio`），你必须使用**公共 URL**。私有的 `*.railway.internal` 主机名在你的本地机器上无法解析。
</Callout>

#### 设置连接字符串变量

在项目根目录创建一个 `.env` 文件，并添加 `DATABASE_URL` 环境变量。使用来自 Railway 的**公共**连接字符串：
...
**Drizzle config** - 一个由 [Drizzle Kit](/docs/kit-overview) 使用的配置文件，包含数据库连接、迁移文件夹和 schema 文件的所有信息。
...
`generate` 命令仅会根据你的 schema 创建迁移 SQL 文件——它**不会**将任何更改应用到数据库。这些迁移会按照 `drizzle.config.ts` 中的配置存储在 `migrations` 目录中。该目录将包含更新数据库 schema 所需的 SQL 文件，以及用于存储不同迁移阶段 schema 快照的 `meta` 文件夹。
...
另外，你也可以使用 [Drizzle kit push 命令](/docs/kit-overview#prototyping-with-db-push)直接将更改推送到数据库：
...
This is the project’s basic file structure. In the `src` directory, we include files related to the database, including the connection in `db.ts` and the schema definitions in `schema.ts`.
...
一旦你[生成了迁移](#applying-changes-to-the-database)，请将 `migrations` 目录提交到你的仓库——当应用启动时，这些文件将会自动应用。
...
<Callout type="info">
`${{Postgres.DATABASE_URL}}` 引用变量会在运行时自动解析为**私有**内部连接字符串，这对于 Railway 内部服务之间的通信是最优的。这与本地 `.env` 文件中使用的公共 URL 不同。
</Callout>
...
如果你需要**零停机部署**，你可能希望将迁移作为一个**独立步骤**在新版本应用开始接收流量之前执行。这样你就可以独立于数据库更改回滚应用。

在 Railway 上，你可以使用[预部署命令](https://docs.railway.com/deployments/pre-deploy-command)来实现这一点——它在构建和部署阶段之间运行，可以访问你的私有网络和环境变量，如果执行失败，部署将不会继续。
...
有关更多详情，请参阅 [Drizzle migrations fundamentals](/docs/migrations) 页面。
...
你可以将 [Drizzle Studio](/drizzle-studio/overview) 与你的应用一起部署到 Railway，从浏览器中直接浏览和管理你的数据库。你可以使用 [Drizzle Studio Railway 模板](https://railway.com/deploy/drizzle-studio-1)，或者按照以下步骤操作。
