...
本教程演示如何将 Drizzle ORM 与 [Node.js](https://nodejs.org/) 和 PostgreSQL 数据库一起使用，全部部署在 [Railway](https://driz.link/railway) 上。

<Prerequisites>
  - 你应该已经安装了 [Node.js](https://nodejs.org/)。你可以按照 [官方指南](https://nodejs.org/en/download/) 进行安装。

  - 你应该已经安装了 Drizzle ORM 和 [Drizzle kit](/docs/kit-overview)。你可以通过运行以下命令来完成：
  <Npm>
      drizzle-orm@rc
      -D drizzle-kit@rc
  </Npm>

  - 你应该已经安装了 `pg` 包作为 PostgreSQL 驱动，以及 `tsx` 用于直接运行 TypeScript 文件。
  <Npm>
    pg
    -D @types/pg tsx
  </Npm>

  - 你应该拥有一个 [Railway](https://driz.link/railway) 账户。
</Prerequisites>
...
This is the project’s basic file structure. In the `src` directory, we have files related to the database, including the connection in `db.ts` and the schema definitions in `schema.ts`.
...
你可以将 [Drizzle Studio](/drizzle-studio/overview) 与你的应用一起部署到 Railway，这样就能直接在浏览器中浏览和管理数据库。你可以使用 [Drizzle Studio Railway 模板](https://railway.com/deploy/drizzle-studio-1)，或者按照下面的步骤操作。
