...
Drizzle 支持对 Postgres 策略和角色进行原始表示，并且可以以您想要的任何方式使用。这适用于流行的 Postgres 数据库提供商，例如 `Neon` 和 `Supabase`。

在 Drizzle 中，我们为两个数据库提供者提供了特定的预定义 RLS 角色和函数，但您也可以定义自己的逻辑。

## 启用 RLS

如果你只是想在表上启用 RLS，而不添加策略，可以使用 `pgTable.withRLS(...)`
...
如果你向表中添加策略，RLS 将会自动启用。因此，在向表中添加策略时，无需显式启用 RLS。
</Callout>

## 角色

目前，Drizzle 支持通过几种不同的选项定义角色，如下所示。未来版本将添加对更多选项的支持。
...
如果数据库中已经存在某个角色，并且你不希望 drizzle-kit “看到”它或将其包含在迁移中，你可以将该角色标记为已存在。
...
在这些情况下，您可以在 `drizzle.config.ts` 中使用 `entities.roles`。有关完整参考，请参阅 [drizzle.config.ts](/docs/drizzle-config-file) 文档。
...
在这种情况下，您可以使用 `provider` 选项和 `exclude` 排除其他角色：
