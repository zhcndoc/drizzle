...
如果您需要 `generate` 命令跳过可交换性检查并绕过它，可以使用 `--ignore-conflicts`。如果您想使用它，那么
很大概率是 `drizzle-kit` 没有正确检查迁移，这是一个 bug。请将您的情况报告给我们，以便我们修复它
...
| `driver`      |            | 驱动之一：`aws-data-api`、`pglite`   |
| `out`         |            | 迁移输出文件夹，默认值为 `./drizzle`                           |
