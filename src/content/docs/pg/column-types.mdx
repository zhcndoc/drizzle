...
我们对所有这些都提供原生支持，如果这些还不够满足您的需求，欢迎创建 **[自定义类型](/docs/custom-types)**。

<Callout title='重要' type='warning'>
本部分文档中的所有示例都不使用数据库列名别名，列名是从 TypeScript 键生成的。

如果您愿意，也可以在列名中使用数据库别名，并且还可以使用 `casing API` 为 Drizzle 定义映射策略。
您可以在[这里](/docs/sql-schema-declaration#shape-your-data-schema)了解更多。
</Callout>
...
PostgreSQL 提供标准 SQL 类型 bytea。

有关更多信息，请参阅官方 PostgreSQL **[文档.](https://www.postgresql.org/docs/current/datatype-binary.html)**
...
### 固定字符
`character(n)` `char(n)`  
固定长度、带空格填充的字符串，可以存储最多 **`n`** 个字符（不是字节）。
...
UUID 数据类型，存储符合 RFC 4122、ISO/IEC 9834-8:2005 及相关标准的全局唯一标识符。

有关更多信息，请参阅官方 PostgreSQL **[文档.](https://www.postgresql.org/docs/current/datatype-uuid.html)**
...
- `abc` 将被接受插入并在选择时映射为一个具有来自方程 `Ax + By + C = 0` 的 a、b、c 常量的对象。因此，数据库的 Line `{1,2,3}` 将被 Drizzle 视为 `{ a: 1, b: 2, c: 3 }`。
...
### inet
`inet`  
`inet` 类型保存一个 IPv4 或 IPv6 主机地址，并可选地包含其子网，全部存储在一个字段中

更多信息请参阅 PostgreSQL 官方 **[文档。](https://www.postgresql.org/docs/current/datatype-net-types.html#DATATYPE-INET)**
...
### cidr
`cidr`  
`cidr` 类型保存一个 IPv4 或 IPv6 网络规格

更多信息请参阅 PostgreSQL 官方 **[文档。](https://www.postgresql.org/docs/current/datatype-net-types.html#DATATYPE-CIDR)**
...
### macaddr
`macaddr`  
`macaddr` 类型存储 MAC（媒体访问控制）地址

更多信息请参阅 PostgreSQL 官方 **[文档。](https://www.postgresql.org/docs/current/datatype-net-types.html#DATATYPE-MACADDR)**
...
### macaddr8
`macaddr8`  
`macaddr8` 类型以 EUI-64 格式存储 MAC 地址

更多信息请参阅 PostgreSQL 官方 **[文档。](https://www.postgresql.org/docs/current/datatype-net-types.html#DATATYPE-MACADDR8)**
...
PostgreSQL 支持将身份列作为一种自动为列生成唯一整数值的方式。这些值使用序列生成，并且可以使用 `GENERATED AS IDENTITY` 子句进行定义。
