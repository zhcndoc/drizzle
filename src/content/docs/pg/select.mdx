...
它原生支持几乎所有方言中的绝大多数查询特性和能力，
而它目前尚未支持的部分，也可以由用户借助强大的 [sql](/docs/sql) 运算符来添加。

对于下面的示例，假设您有一个像这样定义的 `users` 表：
...
### 高级选择
借助 TypeScript，Drizzle API 让您可以以多种灵活方式构建 select 查询。
...
[基于游标的分页](/docs/guides/cursor-based-pagination) 指南。
...
<Callout type="info">
`cast(... as int)` 是必要的，因为在 PostgreSQL 中 `count()` 返回 `bigint`/`string`。
或者，您也可以使用 [`.mapWith(Number)`](/docs/sql#sqlmapwith) 在运行时将值转换为数字。

如果您需要 count 聚合 - 我们建议使用 [$count](/docs/select#count) API
</Callout>
