...
Drizzle ORM 设计为建立在 SQL 之上的一个轻量级类型化层。  
我们真心相信，我们已经设计出了在 TypeScript 中操作 SQL 数据库的最佳方式，现在是时候让它变得更好了。  
...
在关系查询中，对表列的引用必须通过回调参数来进行，而不是通过导入的表对象。 
这适用于所有接受回调的子句——`orderBy`、`where`.`RAW`、`extras` 以及 `extras` 内部的 `subqueries`。 
回调会暴露当前查询作用域中已别名化的表，这对于在嵌套或自引用查询中正确生成 SQL 是必需的。
...
<Callout type="info" emoji="ℹ️">
	`drizzle` 的导入路径取决于你正在使用的 **[数据库驱动](/docs/connect-overview)**。
</Callout>
...
使用 Drizzle 关系查询，你不仅可以按查询的表进行过滤，还可以按查询中包含的任何表进行过滤。

**示例：** 获取 ID 大于 10 且至少有一条内容以 "M" 开头的帖子的 `users`：
```ts
...
要检索所有包含评论的帖子，并添加一个附加字段来计算帖子内容的大小以及每条评论内容的大小：
<Section>
```typescript copy
