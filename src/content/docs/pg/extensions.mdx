...
位字符串类型，用于 pgvector 中的二进制向量

有关更多信息，请参阅官方 pg_vector **[docs.](https://github.com/pgvector/pgvector)**
...
<Callout type="info">
如果您正在使用 `introspect` 或 `push` 命令配合 PostGIS 扩展，并且不希望包含 PostGIS 表，您可以使用 [extensionsFilters](/docs/drizzle-config-file#extensionsfilters) 来忽略所有 PostGIS 表
</Callout>
