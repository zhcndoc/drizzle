...
## Type API
使用 Drizzle 的类型辅助工具，从 PostgreSQL 表结构中推断 `select` 和 `insert` 模型。
...
你可以通过创建一个 `DefaultLogger` 实例并向其提供自定义 `writer` 来更改日志输出目标：
```typescript copy
...
## 多项目 schema
当多个项目共享一个数据库时，使用 `pgTableCreator` 自定义表名。
...
## 独立查询构建器
无需创建数据库实例即可使用 PostgreSQL 查询构建器。
...
## 获取类型化列
使用 `getColumns` 获取类型化列映射，这在从选择中排除字段时很有用。
