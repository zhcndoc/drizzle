...
你可以在此处了解更多相关内容 [这里](/docs/sql-schema-declaration#shape-your-data-schema)
</Callout>
...
A data type for integers, whose values can be `1`, `0`, or `NULL`

Drizzle allows using `true` or `false` instead of `1` and `0`
...
Fixed-length binary data, with a length of n bytes, where n ranges from 1 to 8,000. Storage size is n bytes.
...
Variable-length binary data. n can be a value between 1 and 8,000. max means the maximum storage size is 2^31-1 bytes.
...
Fixed precision and scale numeric values. When using the maximum precision, valid values range from -10^38 + 1 to 10^38 - 1.
