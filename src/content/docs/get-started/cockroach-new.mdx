...
<Prerequisites>
  ...
  - **node-postgres** - 用于查询你的 Cockroach 数据库的包 - [在这里阅读](https://node-postgres.com/)
</Prerequisites>

Drizzle 原生支持使用 `node-postgres` 和 `postgres.js` 驱动连接 CockroachDB。

我们将在这个快速入门示例中使用 `node-postgres`。但如果你想了解更多连接 postgresql 的方式，请查看我们的 [CockroachDB Connection](/docs/cockroach/get-started-cockroach) 页面
