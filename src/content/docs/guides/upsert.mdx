...
<IsSupportedChipGroup chips={{PostgreSQL: true, MySQL: true, SQLite: true, MSSQL: false, Cockroach: true}}/>
...
- [Insert statement](/docs/insert)、[onConflictDoUpdate method](/docs/insert#on-conflict-do-update) 和 [onDuplicateKeyUpdate method](/docs/mysql/insert#on-duplicate-key-update)
- [复合主键](/docs/indexes-constraints#composite-primary-key)
- [sql operator](/docs/sql)
</Prerequisites>

<Callout type="warning">
**SQL Server (MSSQL)** 不支持 ***ON CONFLICT DO UPDATE*** 语法
</Callout>

### PostgreSQL、Cockroach 和 SQLite

要在 PostgreSQL、Cockroach 和 SQLite 中使用 Drizzle 实现 upsert 查询（跳转到 [MySQL](/docs/guides/upsert#mysql)），您可以使用 `.onConflictDoUpdate()` 方法：
...
要在 PostgreSQL、Cockroach 和 SQLite 中一次查询上插入多行，您可以使用 `sql operator` 和 `excluded` 关键字。`excluded` 是一个特殊引用，指向原本要插入但由于冲突而未能插入的那一行。
...
Drizzle 提供了简单而灵活的 API，使您可以轻松创建自定义解决方案。以下是在 PostgreSQL、Cockroach 和 SQLite 中，针对冲突更新多行中特定列的自定义函数实现方式：
...
以下是如何在 PostgreSQL、Cockroach 和 SQLite 中实现带有多个目标的 upsert 查询：
...
如果您想在 `update` 语句中实现带有 `where` 条件的上插入查询，可以使用 `onConflictDoUpdate` 方法中的 `setWhere` 属性：
