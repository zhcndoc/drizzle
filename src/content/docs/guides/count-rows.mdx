...
<Prerequisites>
- 入门 [PostgreSQL](/docs/get-started-postgresql)、[MySQL](/docs/mysql/get-started-mysql)、[SQLite](/docs/sqlite/get-started-sqlite)、[MSSQL](/docs/mssql/get-started-mssql) 和 [Cockroach](/docs/cockroach/get-started-cockroach)
- [Select 语句](/docs/select)
- [Filters](/docs/operators) 和 [sql 操作符](/docs/sql)
- [Aggregations](/docs/select#aggregations) 和 [Aggregation helpers](/docs/select#aggregations-helpers)
- [Joins](/docs/joins)
</Prerequisites>
...
Drizzle 具有简单而灵活的 API，这使你能够创建自定义解决方案。在 PostgreSQL、MySQL 和 Cockroach 中，`count()` 函数返回 bigint，而驱动程序会将其解释为字符串，因此应将其转换为 integer：
...
在 SQLite 和 MSSQL 中，`count()` 的结果会作为 integer 返回。
