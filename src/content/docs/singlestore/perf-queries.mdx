# 查询性能
当谈到 **Drizzle** 时——我们是在 SQL 之上的一层很薄的 TypeScript 层，几乎没有任何开销；如果想让它真正做到 0 开销，你可以使用我们的预编译语句 API。
...
使用预编译语句时，你只需在 Drizzle ORM 这一侧进行一次 SQL 拼接，然后数据库驱动就能够重用预编译的二进制 SQL，而不是每次都解析查询。  
这对大型 SQL 查询有极其显著的性能优势。  
...
## 占位符
当你需要嵌入一个动态运行时值时，可以使用 `sql.placeholder(...)` API
