...
<Prerequisites>  
  - **关系基础** - 熟悉外键约束、软关系、数据库规范化等概念 - [点击这里阅读](/docs/relations-schema-declaration)
  - **声明 schema** - 熟悉如何定义 drizzle schemas - [点击这里阅读](/docs/sql-schema-declaration)
  - **数据库连接** - 熟悉如何使用 drizzle 连接数据库 - [点击这里阅读](/docs/get-started-postgresql)
</Prerequisites>
...
- `feed` 键是在使用 Drizzle 关系查询时出现在 `users` 对象中的自定义键。
- `r.many.posts` 表示 `feed` 将是来自 `posts` 表的对象数组，而不仅仅是一个对象
- `from: r.users.id` 指定了我们建立软关联的起始表。 
此处，关联从 `users` 表的 `id` 列开始。
- `to: r.posts.ownerId` 指定了我们建立软关联的目标表。 
此处，关联指向 `posts` 表的 `ownerId` 列。
- `alias` 用于给两个表之间的关系添加特定别名。如果两个表之间有多个相同的关系，应使用 `alias` 进行区分
- `where` 条件可用于多态关系。它基于 `where` 语句获取关系。
例如在上述示例中，只会获取 `approved` 帖子。更多关于多态关系，见 [这里](/docs/relations-schema-declaration#polymorphic-relations)。
