...
<Callout title='警告'>
目前，在支持它的生成器中同时指定 `arraySize` 和 `isUnique`，将会生成唯一值（而不是唯一数组），随后这些值会被打包成数组。
</Callout>
...
如果只提供其中一个参数（`minDate` 或 `maxDate`），则未指定的参数将通过对已指定的参数加上或减去 8 年来计算
</Callout>
...
|  | param           | default                                                                                       | type
|:-| :--------       | :--------                                                                                     | :--------
|  |`min`            |``` `'2024-05-08' - 2 years` - 如果未设置 max ``` ``` `max - 4 years`  - 如果已设置 max ```   |`string \| Date`
|  |`max`            |``` `'2024-05-08' + 2 years` - 如果未设置 min ``` ``` `min + 4 years`  - 如果已设置 min ```   |`string \| Date`
|  |`arraySize`      |--                                                                                             |`number`
...
await seed(db, schema, { count: 1000 }).refine((funcs) => ({
  users: {
    columns: {
      phoneNumber: funcs.phoneNumber({
        // `prefixes` - 你想作为电话号码前缀的任意字符串数组。（与 `template` 属性不兼容）
...
// 使用 prefixes 和 generatedDigitsNumbers 属性生成电话号码，但对不同的 prefixes 使用不同的 generatedDigitsNumbers
await seed(db, schema, { count: 1000 }).refine((funcs) => ({
  users: {
    columns: {
      phoneNumber: funcs.phoneNumber({
...
|:-| :--------   | :--------                                                                               | :--------
|  |`isUnique`   |`数据库列唯一性`                                                                     |`boolean`
|  |`maxXValue`  |``` `如果 isUnique 为 false，则为 `10 * 1000``` ``` `如果 isUnique 为 true，则为 `10 * count```  |`number`
|  |`minXValue`  |`-maxXValue`                                                                             |`number`
|  |`maxYValue`  |``` `如果 isUnique 为 false，则为 `10 * 1000``` ``` `如果 isUnique 为 true，则为 `10 * count```  |`number`
...
await seed(db, schema, { count: 1000 }).refine((funcs) => ({
  bitStringTable: {
    columns: {
      bit: funcs.bitString({
        // 每个位字符串的期望长度（例如，`dimensions = 3` 生成像 `'010'` 这样的值）。
        dimensions: 12,
