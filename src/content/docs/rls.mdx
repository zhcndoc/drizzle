...
从 `v1.0.0-beta.1` 开始，`.enableRLS()` 已被弃用，
如果您只想在表上启用 RLS 而不添加策略，可以使用 `pgTable.withRLS(...)`
...
`getColumns` 从 `drizzle-orm@1.0.0-beta.2` 开始可用（详细请参阅 [这里](/docs/upgrade-v1)）

如果您使用的是 1.x 之前的版本（如 `0.45.1`），请使用 `getTableColumns`
</Callout>
