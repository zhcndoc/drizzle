...
<Callout title='警告'>
目前，在支持 `arraySize` 的生成器中同时指定 `isUnique`，将会生成唯一值（而不是唯一数组），然后这些值会被打包成数组。
</Callout>
...
|:-| :--------  | :--------                                                                          | :--------
|  |`isUnique`  |`数据库列唯一性`                                                        |`boolean`
|  |`maxValue`  |``` `isUnique 为 false 时为 1000` ``` ``` `isUnique 为 true 时为 count * 10` ```  |`number \| bigint`
...
|  | param      | default                                                                                                   | type
|:-| :--------  | :--------------                                                                                           | :--------
|  |`minDate`   |``` `'2024-05-08' - 4 years` - 如果未设置 maxDate``` ``` `maxDate - 8 years`  - 如果已设置 maxDate ```   |`string \| Date`
|  |`maxDate`   |``` `'2024-05-08' + 4 years` - 如果未设置 minDate``` ``` `minDate + 8 years`  - 如果已设置 minDate ```   |`string \| Date`
|  |`arraySize` |--                                                                                                         |`number`

<Callout type='warning'>
如果只提供了其中一个参数（`minDate` 或 `maxDate`），未指定的参数将通过对已指定参数加上或减去 8 年来计算
</Callout>
...
|  | param           | default                                                                                       | type
|:-| :--------       | :--------                                                                                     | :--------
|  |`min`            |``` `'2024-05-08' - 2 years` - 如果未设置 max ``` ``` `max - 4 years`  - 如果设置了 max ```   |`string \| Date`
|  |`max`            |``` `'2024-05-08' + 2 years` - 如果未设置 min ``` ``` `min + 4 years`  - 如果设置了 min ```   |`string \| Date`
|  |`arraySize`      |--                                                                                             |`number`
  ...
  users: {
    columns: {
      phoneNumber: funcs.phoneNumber({ 
        // `template` - 电话号码模板，其中所有 `#` 符号都会被生成的数字替换。
        template: "+(380) ###-####",
...
await seed(db, schema, { count: 1000 }).refine((funcs) => ({
  users: {
    columns: {
      phoneNumber: funcs.phoneNumber({
        // `prefixes` - 任意字符串数组，你希望它们作为电话号码前缀。（与 `template` 属性不兼容）
...
<rem025 />
在指定范围内为 x 和 y 坐标生成二维点。

|  | param       | default                                                                                 | type
|:-| :--------   | :--------                                                                               | :--------
|  |`isUnique`   |`数据库列唯一性`                                                             |`boolean`
|  |`maxXValue`  |``` `若 isUnique 等于 false，则为 10 * 1000` ``` ``` `若 isUnique 等于 true，则为 10 * count` ```  |`number`
|  |`minXValue`  |`-maxXValue`                                                                             |`number`
|  |`maxYValue`  |``` `若 isUnique 等于 false，则为 10 * 1000` ``` ``` `若 isUnique 等于 true，则为 10 * count` ```  |`number`
