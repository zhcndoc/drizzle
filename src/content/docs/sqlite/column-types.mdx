...
我们原生支持它们全部，但如果这还不够用，您也可以随时创建 **[自定义类型](/docs/sqlite/custom-types)**。

<Callout title='important' type='warning'>
本部分文档中的所有示例均未使用数据库列名别名，列名是从 TypeScript 键生成的。

如果您愿意，也可以在列名中使用数据库别名，并且还可以使用 `casing API` 为 Drizzle 定义映射策略。
您可以在[这里](/docs/sqlite/sql-schema-declaration#shape-your-data-schema)了解更多
</Callout>
...
export const table = sqliteTable('table', {
  ...
  numericString: numeric({ mode: 'string' }),
});
```
