...
- Drizzle Studio，我们的数据库浏览器 - [请阅读这里](/drizzle-studio/overview)
</Prerequisites>

`drizzle-kit studio` 命令会启动一个用于 [Drizzle Studio](/drizzle-studio/overview) 的服务器，该服务器托管在 [local.drizzle.studio](https://local.drizzle.studio) 上。 
它要求您通过 [drizzle.config.ts](/docs/cockroach/drizzle-config-file) 配置文件指定数据库连接凭据。
...
**Drizzle Studio 组件** - 是一个预打包的、与框架无关的 Drizzle Studio 网页组件， 
