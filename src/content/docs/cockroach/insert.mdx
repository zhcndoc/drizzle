...
你可以在 CockroachDB 中插入一行并将其取回，如下所示：
```typescript copy
...
使用复合索引，或用于 `onConflictDoUpdate` 的复合主键进行 upsert：
...
<Callout>
  查看如何在 [select](/docs/cockroach/select#with-clause)、[update](/docs/cockroach/update#with-update-clause)、[delete](/docs/cockroach/delete#with-delete-clause) 中使用 WITH 语句
</Callout>

使用 `with` 子句可以通过将复杂查询拆分为更小的子查询（称为公用表表达式，CTE）来帮助你简化它们：
<Section>
```typescript copy
...
正如 CockroachDB 文档所述：
<Callout>
提供要插入的行的查询（SELECT 语句）
</Callout>

Drizzle 支持所有方言的当前语法，并且它们都共享相同的语法。让我们回顾一些常见场景和 API 用法。 
在插入语句中使用 select 有几种方式，可以让你选择自己偏好的方法：

- 你可以在 select 函数中传入一个查询构建器。
- 你可以在回调中使用一个查询构建器。
- 你可以传入一个 SQL 模板标签，并使用你想要的任何自定义 select 查询
