...
目前，在支持它的生成器中同时指定 `arraySize` 和 `isUnique` 将会生成唯一值（而不是唯一数组），然后这些值会被打包成数组。
</Callout>
...
|:-| :--------  | :--------                                                                          | :--------
|  |`isUnique`  |`数据库列唯一性`                                                        |`boolean`
|  |`maxValue`  |``` `如果 isUnique 为 false，则为 1000` ``` ``` `如果 isUnique 为 true，则为 count * 10` ```  |`number \| bigint`
...
如果只提供其中一个参数（`minDate` 或 `maxDate`），未指定的参数将通过对已指定的参数加上或减去 8 年来计算得出
</Callout>
...
|:-| :--------                | :--------                                       | :--------
|  |`template`                |--                                               |`string`
|  |`prefixes`                |[前缀使用的数据集](https://github.com/OleksiiKH0240/drizzle-orm/blob/main/drizzle-seed/src/datasets/phonesInfo.ts)   |`string[]`
  ...
  users: {
    columns: {
      phoneNumber: funcs.phoneNumber({
        ...
        // `generatedDigitsNumbers` - 将添加到前缀末尾的位数。（与 `template` 属性不兼容）
...
<Tab>
目前，如果你将 arraySize 设置为大于 1 的值，  
或者尝试在 CockroachDB 中，或通过 drizzle-orm 在 CockroachDB 中向 `geometry(point, 0)[]` 列插入多个 `geometry point` 元素，  
你会遇到错误。
...
<Tab>
目前，如果你在 drizzle-orm 的表声明中将 `geometry(point)` 列的 SRID 设置为 0 以外的任何值（例如 4326），  
在播种过程中你会遇到错误。
  ...
  geometryTable: {
    columns: {
      geometryPointTuple: funcs.geometry({
        ...
        // 要生成的几何类型；目前仅支持 `'point'`；
        type: "point",

        // 空间参考系统标识符：决定将生成哪种类型的点——`4326` 或 `3857`；
        srid: 4326,
  ...
  vectorTable: {
    columns: {
      vector: funcs.vector({
        ...
        // 每个向量元素的小数位数（例如，`decimalPlaces = 3` 会生成像 `1.123` 这样的值）；
        decimalPlaces: 5,

        // 每个生成向量中的元素数量（例如，`dimensions = 3` 会生成像 `[1,2,3]` 这样的值）；
        dimensions: 12,
