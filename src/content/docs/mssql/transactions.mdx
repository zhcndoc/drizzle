...
SQL 事务是一个或多个 SQL 语句的组合，这些语句与数据库进行交互。
整个事务可以作为一个单一的逻辑单元提交到数据库，
或者回滚（恢复为未执行状态）为一个单一的逻辑单元。
...
await db.transaction(async (tx) => {
  ...
  await tx.transaction(async (tx2) => {
    await tx2.update(users).set({ name: "Dan 先生" }).where(eq(users.name, "Dan"));
  });
});
```
...
你可以将事务与 **[关系查询](/docs/mssql/rqb)** 一起使用：
