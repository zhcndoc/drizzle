...
- [插入语句](/docs/insert)、[onConflictDoUpdate 方法](/docs/insert#on-conflict-do-update)、[onDuplicateKeyUpdate 方法](/docs/insert#on-duplicate-key-update)
...
要在 PostgreSQL 和 SQLite 中使用 Drizzle 实现 upsert 查询（请跳到 [MySQL](/learn/guides/upsert#mysql)），你可以使用 `.onConflictDoUpdate()` 方法：
...
要在 PostgreSQL 和 SQLite 中在一个查询中 upsert 多行，可以使用 `sql 操作符` 和 `excluded` 关键字。`excluded` 是一个特殊引用，指向由于冲突而未被插入的提议插入行。
...
Drizzle 的 API 简单灵活，可以轻松创建自定义解决方案。下面是如何在 PostgreSQL 和 SQLite 中由于冲突而更新多行中的特定列的自定义函数：
...
要在 PostgreSQL 和 SQLite 中实现具有多个目标的 upsert 查询：
...
await db
  ...
  .onConflictDoUpdate({
    ...
    set: { quantity: sql`${inventory.quantity} + 100` }, // 将 100 添加到现有数量
  });
```
...
如果您想为 `update` 语句的 `where` 子句实现 upsert 查询，可以在 `onConflictDoUpdate` 方法的 `setWhere` 属性中使用 `sql 操作符`：
...
await db
  ...
  .onConflictDoUpdate({
    target: users.id,
    set: { ...data, email: sql`${users.email}` }, // 将 email 保留为原样
});
```
...
要在 MySQL 中使用 Drizzle 实现 upsert 查询，可以使用`.onDuplicateKeyUpdate()` 方法。MySQL 将根据主键和唯一索引自动确定冲突目标，并在发生任何唯一索引冲突时更新行。
...
要在 MySQL 中在一个查询中 upsert 多行，可以使用 `sql 操作符` 和 `values()` 函数。`values()` 函数引用了如果没有发生重复键冲突，将要插入的列的值。
...
Drizzle 的 API 简单灵活，可以轻松创建自定义解决方案。下面是如何在 MySQL 中由于冲突而更新多行中的特定列的自定义函数：
...
<CodeTabs items={["index.ts", "schema.ts"]}>
  <CodeTab>
    ...
    insert into users (`id`, `last_login`, `active`)
      values
        (1, '2024-03-16T15:23:28.013', true),
        (2, '2024-03-16T16:23:28.013', true),
        (3, '2024-03-16T17:23:28.013', true)
