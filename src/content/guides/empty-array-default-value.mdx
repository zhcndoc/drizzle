...
要在 PostgreSQL 中将空数组设置为默认值，可以使用 `sql` 运算符配合 `'{}'` 或 `ARRAY[]` 语法：
...
MySQL 没有数组数据类型，但可以使用 `json` 数据类型实现相同的目的。要在 MySQL 中将空数组设置为默认值，可以使用 `JSON_ARRAY()` 函数或 `sql` 运算符配合 `('[]')` 语法：
...
`mode` 选项定义了在应用程序中如何处理值。在 `json` 模式下，值会被当作 JSON 对象字面量处理。

你可以为 json 对象推断指定 `.$type<..>()`，它不会检查运行时的值。它为默认值、插入和选择模式提供了编译时的保护。

### SQLite

SQLite 没有数组数据类型，但可以使用 `text` 数据类型实现相同的目的。要在 SQLite 中将空数组设置为默认值，可以使用 `json_array()` 函数或 `sql` 运算符配合 `'[]'` 语法：
