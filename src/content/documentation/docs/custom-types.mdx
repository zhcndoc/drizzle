...
了解 `customType` 函数如何使用的最佳方式，是查看如何使用 Drizzle ORM 的 `customType` 函数定义现有的 PostgreSQL 和 MySQL 数据类型。

<Tabs items={['PostgreSQL 数据类型', 'MySQL 数据类型']}>
  <Tab>
...
所有类型的使用方法与 Drizzle ORM 中定义的函数相同。例如:
