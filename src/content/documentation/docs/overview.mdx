...
> Drizzle 是一个好朋友，在你需要的时候会支持你，而在你需要一些空间时不会打扰你。

它看起来简单，感觉简单，在你项目的第 _1000_ 天表现出色，\
让你以自己的方式做事情，且在你需要的时候总是在那里。

**它是唯一一个同时拥有 [关系](/docs/rqb) 和 [类似 SQL](/docs/select) 查询 API 的 ORM**，
在访问你的关系数据时，为你提供了两全其美。
Drizzle 轻量、高效、安全、无乳糖、无麸质、清醒、灵活，且 **天生支持无服务器**。
