...
- [Select语句](/docs/select)和[select from子查询](/docs/select#select-from-subquery)
- [内连接](/docs/joins#inner-join)
- [过滤运算符](/docs/operators)和[exists函数](/docs/operators#exists)
</Prerequisites>

该指南演示了如何选择至少有一个相关子行的父行。下面是模式定义和相应的数据库数据示例：
...
要仅选择至少有一个相关子行的父行，可以使用具有`exists()`函数的子查询，如下所示：
