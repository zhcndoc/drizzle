...
## 运行迁移
我们在如何运行迁移方面没有固定意见，你可以使用生成的 SQL 文件手动运行迁移，使用外部工具运行迁移等等，
或者使用 **Drizzle Kit** 或 **Drizzle ORM**。

**使用 Drizzle Kit**

你可以使用本地迁移 CLI 命令 `drizzle-kit migrate`。它将从 `drizzle.config.ts` 文件中收集所有所需的信息来运行迁移。

你可以在 `drizzle.config.ts` 中指定自定义的 schema 和 migration 表名。
更多信息，请查看迁移[文档](/kit-docs/commands#apply-migrations)。
