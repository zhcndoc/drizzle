...
使用 Drizzle ORM 与 Vercel Edge 函数时，您必须使用 Edge 兼容驱动程序，因为函数在 [Edge 运行时](https://vercel.com/docs/functions/runtimes/edge-runtime) 中运行，而不是在 Node.js 运行时，因此标准的 Node.js API 存在一些限制。

根据您的数据库方言，您可以选择下列任一驱动程序：

- [Neon 无服务器驱动](/docs/get-started-postgresql#neon) 允许您访问任何 `PostgreSQL` 客户端，并在无服务器和边缘环境上通过 HTTP 或 WebSockets 查询数据。我们建议为 `非-Vercel Postgres` 客户端使用此驱动程序，因为 Vercel Postgres 驱动程序实现了特定的连接字符串检查，可能与其他 `PostgreSQL` 客户端不兼容。
- [Vercel Postgres 驱动程序](/docs/get-started-postgresql#vercel-postgres) 建立在 `Neon 无服务器驱动` 之上，允许您访问任何 `PostgreSQL` 客户端。我们建议为 `Vercel Postgres` 客户端使用此驱动程序。
- [PlanetScale 无服务器驱动](/docs/get-started-mysql#planetscale) 允许您访问任何 `MySQL` 客户端，并通过 HTTP 连接执行查询，这通常不被云提供商阻止。
