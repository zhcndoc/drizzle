...
检测结果将生成一个 `schema.ts` 文件、一个包含数据库 schema 快照的 `meta` 文件夹、一个包含迁移的 sql 文件，以及一个用于[关系查询](/docs/mssql/rqb)的 `relations.ts` 文件。
...
在[文档](/docs/mssql/drizzle-kit-pull)中了解更多关于检测的信息。
