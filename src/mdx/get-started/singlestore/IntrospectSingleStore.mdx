Drizzle Kit 提供了一条 CLI 命令来探查您的数据库并生成带有迁移的模式文件。模式文件包含关于您数据库表、列、关系和索引的所有信息。

例如，您在数据库中有这样的表：

```sql copy
CREATE TABLE `users_table` (
	`id` int NOT NULL,
	`name` varchar(255) NOT NULL,
	`age` int NOT NULL,
	`email` varchar(255) NOT NULL,
	CONSTRAINT `users_table_id` PRIMARY KEY(`id`),
	CONSTRAINT `users_table_email_unique` UNIQUE(`email`)
);
```

拉取您的数据库模式：

```bash copy
npx drizzle-kit pull --init
```

检查的结果将是一个 `schema.ts` 文件、包含数据库模式快照的 `meta` 文件夹、带有迁移的 sql 文件，以及用于[关系查询](/docs/singlestore/rqb-v2)的 `relations.ts` 文件。

以下是生成的 `schema.ts` 文件的示例：

```typescript copy filename="src/db/schema.ts"
// 通过检查生成的表模式
import { singlestoreTableTable, int, varchar, uniqueIndex } from "drizzle-orm/singlestore-core"
import { sql } from "drizzle-orm"

export const usersTable = singlestoreTable("users_table", {
	id: int().primaryKey().notNull(),
	name: varchar({ length: 255 }).notNull(),
	age: int().notNull(),
	email: varchar({ length: 255 }).notNull(),
},
(table) => [
	uniqueIndex("users_table_email_unique").on(table.email),
]);
```

在[文档](/docs/singlestore/drizzle-kit-pull)中了解有关检查的更多信息。
