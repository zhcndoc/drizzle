...
- [x] 迁移到 folder v3，移除 journal
- [x] 为 push 添加 explain 模式
- [x] 添加可交换迁移检查（PostgreSQL、MySQL）
- [x] 添加可交换迁移检查（SQLite、MSSQL）
- [x] 支持 `generated` 列 [beta 分支]
- [x] MySQL `.$returningIds()`
- [x] 支持 PostgreSQL `decimals` 的 `mode: number` 和 `mode: bigint`
- [x] 使用 `(t) => ` 回调的表声明，可选择性省略列类型导入
- [x] 表声明中为列提供可选数据库别名 -> `id: serial()`
- [x] 支持 PostgreSQL RLS
- [x] Relational Queries V2 API
- [x] `drizzle('pg', ...)` 驱动实例化，以降低入门学习曲线
- [x] 支持 CockroachDB（可与严格 SQL migrations 配合使用）
- [x] 原生 seeding
- [x] 修复：[#2575](https://github.com/drizzle-team/drizzle-orm/issues/2575), [#2572](https://github.com/drizzle-team/drizzle-orm/issues/2572), [#2571](https://github.com/drizzle-team/drizzle-orm/issues/2571),
...
- [x] 支持 MSSQL
- [ ] 🎉 V1 发布流 🎉
- [ ] 下行迁移、更好的回滚，以及对 Drizzle Kit 中 `migrate` 体验的改进
- [ ] 为内嵌 `.env` consumer 提供 `NODE_ENV` 支持
- [ ] 支持 MariaDB
